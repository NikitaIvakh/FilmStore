...
namespace FilmStore.Presentation.Controllers
{
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> AddItem(int filmId, int count = 1)
        {
            await _orderService.AddFilmAsync(filmId, count);
            return RedirectToAction("Index", "Film", new { id = filmId });
        }

        [HttpPost]
        public async Task<IActionResult> UpdateItem(int filmId, int count)
        {
            var model = await _orderService.UpdateFilmAsync(filmId, count);
        ...
        {
            var model = await _orderService.RemoveFilmAsync(filmId);
            return View("Index", model);
        }
